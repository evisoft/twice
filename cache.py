...
class InternalCache(Cache):
    ...
    def set(self, dictionary, exptime = None):
        for key, val in dictionary.iteritems():
            element = {
                'expires_on' : time.time() + (exptime or 0),
                'element' : val
            }
...
class PylibmcCache(Cache):
    ...
    def __init__(self, config):
        ...
        log.msg('Creating connections to twice memcached servers %s...' % ','.join(servers))
        try:
            import mc
            self.cache = mc.Mc(servers, pool_size=connection_pool_size)
            log.msg('twice memcached OK')
        except:
            log.msg('ERROR: Failed to connect to twice memcached')
    ...
    def set(self, dictionary, time = None):
        ...
        if len(pickled_dict):
            return self.cache.set_multi(pickled_dict, time or 0)
        else:
            return {}
    ...
    def get(self, keylist):
        ...
        d = self.cache.get_multi(md5list)
        return d.addCallback(self._format, keylist, md5list)
        
    def delete(self, keylist):
        for key in keylist:
            self.cache.delete(hashlib.md5(key).hexdigest())
